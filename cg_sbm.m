function Rslt = cg_sbm(tOmega,Mat,Option)
% approximate nonlinear conjugate gradient in canonical
% parameters (i.e., natural parameters)
    ...
    while true
        ...
        tG = bsxfun(@minus,g(:,1:Option.K-1),g(:,end)).*tTheta(:,1:Option.K-1);
        tG = tG - bsxfun(@times,sum(tG,2),tTheta(:,1:Option.K-1));
        tGnorm2 = norm(tG,'fro')^2;
